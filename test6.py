...
def display_data(locations):
    ...
    # Create buttons with icons in the scrollable frame
    for location in locations:
        if location in icons:
            btn = tk.Button(scrollable_frame, image=icons[location], command=lambda loc=location: on_location_click(loc, tree))
            btn.pack(side=tk.TOP, padx=5, pady=5)
        else:
            btn = tk.Button(scrollable_frame, text=location, command=lambda loc=location: on_location_click(loc, tree))
            btn.pack(side=tk.TOP, padx=5, pady=5)
